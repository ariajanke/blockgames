...
const ColorGrid & builtin_background();
...
static const VectorI k_special_blocks_location(0, k_block_size*k_color_group_size*2);

std::tuple<sf::Color, VectorI> get_color_group_info(int color) {
    using sf::Color;
...
/* going private! */ static TileEdges get_edges_for(const ConstSubGrid<int> & blocks, VectorI r) {
...
const sf::Texture & load_builtin_background_texture() {
    ...
    rv = std::make_unique<sf::Texture>();
    rv->loadFromImage(to_image(builtin_background()));
    return *rv;
}
...
const uint8_t * get_icon_image() {
    ...
    const auto & background = builtin_background();
...
const ColorGrid & builtin_background() {
    ...
    rv->set_size(k_block_size, k_block_size);
    add_builtin_background(SubGrid<sf::Color>(*rv, VectorI(), k_block_size, k_block_size));
...
const ColorGrid & builtin_blocks() {
    ...
    grid.set_size( k_color_group_size*3     *k_block_size,
                  (k_color_group_size*2 + 1)*k_block_size, std::move(c));
    ...
    add_specials(SubGrid<sf::Color>(grid, VectorI(0, k_color_group_size*2*k_block_size)));
